...
#  echo "max iterations : ${max_iterations}"
  ...
  for ((prognum=1; prognum<=num_progs; prognum++)) do
    ...
    for ((threads=min_threads; threads<=max_threads; threads+=thread_step)) do
      export OMP_NUM_THREADS=$threads
      local omp_label="${label} ${progname} t=$threads"
      local kind="dynamic"
      for ((num_chunks=min_chunks; num_chunks<=max_chunks; num_chunks+=chunks_step)) do
        export OMP_SCHEDULE=$kind,$num_chunks
        full_label="${omp_label}; schedule=$kind;$num_chunks"
